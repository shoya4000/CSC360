...
void* producer (void* v) {
  for (int i = 0; i < NUM_ITERATIONS; i++) {
    ...
    while (items == MAX_ITEMS) {
      producer_wait_count++;
      uthread_cond_wait(start_arg->produce);
    }
    items++;
    histogram[items]++;
    uthread_cond_broadcast(start_arg->consume);
    uthread_mutex_unlock(start_arg->lock);
  }
  return NULL;
}
...
void* consumer (void* v) {
  for (int i = 0; i < NUM_ITERATIONS; i++) {
    ...
    items--;
    histogram[items]++;
    uthread_cond_broadcast(start_arg->produce);
    uthread_mutex_unlock(start_arg->lock);
  }
  return NULL;
}
